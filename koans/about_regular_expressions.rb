...
class AboutRegularExpressions < EdgeCase::Koan
  ...
  def test_asterisk_means_zero_or_more
    ...
    #
    #------------???
    #------------maybe when there's a precondition that's not met
  end
  ...
  # Why?
  #
  #--------because they match as many as they can
  ...
  #
  #-------[] ors the individual characters, while (|) ors entire constructs
  ...
  def test_sub_is_like_find_and_replace
    assert_equal "one foo-three", "one two-three".sub(/(t\w*)/, "foo")
    assert_equal "one foo-three", "one two-three".sub(/(t\w*)/) { "foo" }
    assert_equal "one two-three", "one two-three".sub(/(t\w*)/) { $1 }
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] }
  end

  def test_gsub_is_like_find_and_replace_all
    assert_equal "one foo-foo", "one two-three".gsub(/(t\w*)/, "foo")
    assert_equal "one foo-foo", "one two-three".gsub(/(t\w*)/) { "foo" }
    assert_equal "one two-three", "one two-three".gsub(/(t\w*)/) { $1 }
